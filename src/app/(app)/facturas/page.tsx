...
import { FACTURAS, type EstadoFactura, type Factura } from "@/lib/mock";
  ...
  return (
    <section className="anim-in">
      <PageHead
        ...
        subtitulo="Sube una foto y nos encargamos del resto"
      ...
      <div className="mb-4.5 flex cursor-pointer items-center justify-center gap-8 rounded-card border-2 border-dashed border-[#D8CFBE] bg-linear-to-b from-[#FFFDF9] to-[#FBF7EF] p-7 transition-colors hover:border-brand max-md:flex-col max-md:gap-4">
        <div className="text-center">
          ...
          <p className="mt-1 text-[13.5px] text-ink-soft">o elige cómo enviarlas 👉</p>
        </div>
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {visibles.map((f) => (
              <tr
                key={f.id}
                onClick={f.lineasDetalle ? () => setAbierta(f) : undefined}
                className={cn(
                  "border-b border-line transition-colors last:border-none",
                  f.lineasDetalle && "cursor-pointer hover:bg-hover",
                )}
              >
                <Td>
        ...
        <div className="card anim-in mt-3.5 overflow-hidden">
          <div className="flex items-center gap-3.5 border-b border-line px-5.5 py-4.5 max-md:flex-wrap">
            ...
            <div>
              ...
              <small className="text-[12.5px] text-ink-soft">
                {abierta.fecha} · leído automáticamente de la foto
              </small>
            </div>
          ...
          <table className="w-full border-collapse">
            ...
            <tbody>
              ...
              {abierta.lineasOcultas ? (
                <tr className="border-b border-line">
                  <Td colSpan={5} className="text-center text-[12.5px] text-ink-soft">
                    + {abierta.lineasOcultas} líneas más
                  </Td>
                </tr>
              ) : null}
            </tbody>
          ...
          <div className="flex justify-end gap-2.5 px-5.5 py-4">
            ...
            <button
              onClick={() => setAbierta(null)}
              className="cursor-pointer rounded-xl bg-ink px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-black"
            >
              ✓ Validar factura
            </button>
          </div>
