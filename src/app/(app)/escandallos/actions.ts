...
export async function crearPlato(tipo?: "bebida" | "postre"): Promise<Resultado> {
  const db = getDb();
  if (!db) return SIN_BD;
  const nombre = tipo === "bebida" ? "Nueva bebida" : tipo === "postre" ? "Nuevo postre" : "Nuevo plato";
  const emoji = tipo === "bebida" ? "🍹" : tipo === "postre" ? "🍰" : "🍽️";
  try {
