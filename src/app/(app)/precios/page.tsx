...
export default function PreciosPage() {
  ...
  const [seleccionado, setSeleccionado] = useState<Producto>(PRODUCTOS[0]);
...
function Sparkline({ hist, variacion }: { hist: number[]; variacion: number }) {
  ...
  const rng = max - min || 1;
  const pts = hist
    .map((v, i) => `${(i / (hist.length - 1)) * 88 + 1},${24 - ((v - min) / rng) * 20}`)
    .join(" ");
  return (
...
function HistPanel({ producto: p }: { producto: Producto }) {
  ...
  const pts = p.hist.map((v, i) => ({
    x: pad + (i / (p.hist.length - 1)) * (W - 2 * pad),
